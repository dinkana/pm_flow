...
const view = ref<View>(
  store.state.isFinished || window.location.hash.replace('#', '').length > 0 ? 'result' : 'intro'
)
...
const areaLabels: Record<Area, string> = {
  goals: 'Цели и границы', stakeholders: 'Стейкхолдеры', planning: 'Планирование',
  risks: 'Риски', team: 'Команда', quality: 'Качество'
}

const enLabels: Record<Area, string> = {
  goals: 'Goals & Scope', stakeholders: 'Stakeholders', planning: 'Planning',
  risks: 'Risks', team: 'Team', quality: 'Quality'
}
...
const reportText = computed(() => {
  ...
  ;(Object.keys(store.areaScores) as Area[]).forEach(a => lines.push(`${areaLabels[a]}: ${store.areaScores[a]}%`))
  return lines.join('\n')
})
...
const copyReport = async () => {
  ...
  } catch { flashMsg.value = 'Не удалось скопировать' }
...
<template>
  <main class="min-h-screen bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-100 p-6 flex flex-col items-center transition-colors">
    <div class="w-full max-w-lg flex-1 flex flex-col justify-center space-y-6">
      ...
      <div v-if="view === 'intro'" class="text-center space-y-6">
        ...
        <p class="text-sm text-gray-500 dark:text-gray-400 leading-relaxed">
          12 коротких вопросов по 6 ключевым областям. Получи объективную оценку и план действий — без регистрации и отправки данных на сервер.
        </p>
        <button @click="start"
      ...
      <div v-else-if="view === 'quiz'" class="space-y-6">
        ...
        <div class="flex justify-between pt-2">
          ...
          <button @click="store.next" :disabled="store.state.answers[store.currentQuestion.id] === null"
      ...
      <div v-else ref="resultRef" class="space-y-6">
        ...
        <div class="space-y-3">
          <div v-for="(score, area) in store.areaScores" :key="area" class="space-y-1">
            ...
            <div class="h-2 w-full bg-gray-200 dark:bg-gray-700 rounded-full overflow-hidden">
              <div :class="zoneClasses[zone(score)].bar" class="h-full transition-all duration-500"
        ...
        <div v-if="store.recommendations.length > 0" class="space-y-3">
          ...
          <div class="space-y-2">
            <div v-for="(rec, index) in store.recommendations" :key="rec.id"
              ...
              <div v-if="expandedRec === rec.id" class="p-4 bg-white dark:bg-gray-900 space-y-3 border-t border-gray-200 dark:border-gray-700">
                <div>
    ...
    <footer class="w-full max-w-lg text-center text-xs text-gray-500 dark:text-gray-400 mt-8 pt-4 border-t border-gray-200 dark:border-gray-800">
      Инструмент создан для быстрой самодиагностики проектов. Потребовалась глубинная экспертиза?
      <a href="https://t.me/your_username" target="_blank" rel="noopener" class="text-blue-600 dark:text-blue-400 hover:underline">Telegram</a> ·
      <a href="https://linkedin.com/in/your_profile" target="_blank" rel="noopener" class="text-blue-600 dark:text-blue-400 hover:underline">LinkedIn</a>
    </footer>
